...
const singleUpload = upload.single('interested_in_icon')
const connectionImageUpload = upload.single('connection_images')
...
const interestedInController = {
        ...
        if (req.files) {
            ...
            if (req.files.connection_images) {
                for (let i = 0; i < req.files.connection_images.length; i++) {
                    let connectionData = {
                        name: "connection_type/" + req.files.connection_images[i].filename,
                    }
    ...
    addConnectionImage: async(req, res) => {
        connectionImageUpload(req, res, async(err) => {
            ...
            req.body.image = 'connection_type/' + filename
