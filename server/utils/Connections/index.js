...
const connectionUtil = {
    ...
    getHomeConnections: async(data) => {
        ...
        let { user_id, page, interested_in, connection_type, profile_verification, background_check_verification, driving_license_verification, filter_lat, filter_lng, filter_distance, min_age, max_age } = data;
        let user_lat = '';
        let user_lng = '';
        ...
        console.log("user_detail0", user_detail);
        ...
        if (connected.status) {
            connected.data.forEach(element => {
                if (element.requested_to.toHexString() == user_id && element.status == "connected") {
                    ...
                    connectedArray.push(element.requested_by.toHexString());
                } else {
        ...
        if (interested_in && interested_in != "") {
            ...
            console.log("interestedArray", interestedArray);
            queryObject.$or = interestedArray;
        } else if (user_interested_in != "") {

            interestedInArray = user_interested_in.split(',');
        ...
        console.log("=====> query object after and & or", queryObject);
        ...
        // let total_page = 0;
        // let total_users = await getDataArray(Users, queryObject, '');
        // if (total_users.status) {
        //     total_user_count = total_users.data.length;
        //     total_page = total_user_count / 10;
        // }
        ...
        let result = await Users.aggregate(
            [{
                ...
                // { $merge: { into: "myOutput", on: "my_hobbies", whenMatched: "replace", whenNotMatched: "insert" } }


                // $project: {
                //     "intrr": { // merge all tickets into one single array
                //         $merege: [
                //             "$my_hobbies",
                //             "$my_interests"
                //         ]
        ...
        let others = { next_page: false }
            ...
            //     }
            // }
        return helpers.showResponse(true, Messages.USER_DATA_FOUND, homeConnectionArray, others, 200);
    },
    ...
    getParticularUserDetail: async(data) => {
        ...
        if (result.status) {
            ...
            if (left_swiped.status) {
                ...
                let right_swiped = await getSingleData(Connections, { requested_by: ObjectId(user_id), requested_to: ObjectId(particular_user_id) }, '')
    ...
    getMyConnectionList: async(data) => {
        ...
        let result = await getDataArray(Connections, queryObj, '', null, { created_at: -1 }, populate)
        if (result.status) {
