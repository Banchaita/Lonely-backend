...
const userUtil = {
    ...
    register: async(data) => {
        ...
        if (login_source === "phone") {
            ...
            console.log("auth token exist", authTokenCheck)
            if (authTokenCheck.status) {
                ...
                console.log("inside else check if email exist", auth_token, email);
                let emailCheck = await getSingleData(Users, { email, status: { $ne: 2 } }, '');
                if ("email exist", emailCheck);
                if (emailCheck.status) {
                    // email already exist
                    let userObj = {
                        email,
                    ...
                    let response = await updateData(Users, userObj, ObjectId(emailCheck.data._id));
                    if (response.status) {
                        console.log("before_jwt_token", response);
                        let token = jwt.sign({ user_id: response.data._id }, process.env.API_SECRET, {
                            expiresIn: process.env.JWT_EXPIRY
                        });
                        let other = { token }
    ...
    updateUserInterestedIn: async(data) => {
        ...
        // let updateObj = {
        //     interested_in: ObjectId(interested_in)
        // }
        // let response = await updateData(Users, updateObj, ObjectId(user_id));
        // if (response.status) {
        //     return helpers.showResponse(true, Messages.INTERESTED_IN_UPDATE_SUCCESS, response.data, null, 200);
        // }
    ...
    updateSubscription: async(data) => {
        let { user_id, has_subscribed } = data;
        let response = await updateData(Users, { has_subscribed }, ObjectId(user_id));
        if (response.status) {
    ...
    getChatUserDetailsAdmin: async(data) => {
        let { user_id } = data;
        let result = await getSingleData(Users, { _id: ObjectId(user_id) }, '');
        if (result.status) {
            return helpers.showResponse(true, Messages.USER_DATA_FOUND, result.data, null, 200);
        }
        return helpers.showResponse(false, Messages.DATA_NOT_FOUND, null, null, 200);
